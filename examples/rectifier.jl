...
function SourceAC3(;
    ...
    return System(
        eqs, t, [ramp], [RMSVoltage, Phase, Freq, τ];
...
function CircuitBridge(;
        name,
        RMSVoltage = 9.0,
        Freq = 50.0,
        R_load = 10.0,
        I_S = 1.0e-6,    # saturation current, unused in this example
        n_ideal = 1.0,     # ideality factor, unused in this example
        T_K = 293.15
    )  # temperature in Kelvin, unused in this example
    ...
    return System(eqs, t; systems = [src, bridge, gnd, rload], name)
end
...
solBridge = solve(probBridge, Rodas5P(), abstol = 1.0e-9, reltol = 1.0e-12, dense = false)

time    = solBridge.t
v_load  = solBridge[circuitbridge.rload.v]
i_load  = solBridge[circuitbridge.rload.i]
